...
const Hero: React.FC = () => {
  return (
    <div className="relative min-h-[90vh] flex flex-col items-center justify-center py-20 px-6 overflow-hidden bg-white">
      ...
      <div className="absolute inset-0 z-0">
        <video 
          ...
          muted 
          playsInline 
          className="w-full h-full object-cover opacity-60 transition-opacity duration-1000 grayscale-[40%]"
        >
          <source 
            src="https://assets.mixkit.co/videos/preview/mixkit-business-people-having-a-meeting-around-a-table-23653-large.mp4" 
            type="video/mp4" 
          />
          Your browser does not support the video tag.
        </video>
        {/* Soft White Gradient Overlay - High transparency in center to let video through */}
        <div className="absolute inset-0 bg-gradient-to-b from-white/60 via-white/20 to-white/90"></div>
        {/* Secondary Overlay for better contrast on white background */}
        <div className="absolute inset-0 bg-white/20 backdrop-blur-[1px]"></div>
      </div>

      {/* Decorative Diagonal Pattern */}
      <div className="absolute bottom-[-5%] right-[-5%] w-[45%] h-[45%] opacity-10 pointer-events-none z-10">
        <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
          <defs>
            <pattern id="diagonal-stripe-light" patternUnits="userSpaceOnUse" width="10" height="10" patternTransform="rotate(45)">
              <line x1="0" y1="0" x2="0" y2="10" stroke="#000" strokeWidth="1" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#diagonal-stripe-light)" />
        </svg>
      </div>
      ...
      <div className="relative z-20 max-w-[1440px] mx-auto text-center w-full flex flex-col items-center">
        ...
        <div className="mb-12 animate-in fade-in slide-in-from-top-10 duration-1000">
          <h1 className="text-4xl md:text-6xl lg:text-[84px] font-black text-[#1A1A1A] leading-[1.05] tracking-tight uppercase drop-shadow-sm">
        ...
        <div className="w-full overflow-x-auto no-scrollbar mb-16 px-4 animate-in fade-in slide-in-from-bottom-6 duration-1000 delay-300">
          <div className="flex flex-nowrap md:flex-wrap items-center justify-start md:justify-center gap-4 py-2">
            {specialties.map((spec, idx) => (
              <div 
                key={idx} 
                className="flex items-center gap-3 bg-white/95 backdrop-blur-md border border-gray-100 px-6 py-4 rounded-2xl shadow-lg hover:shadow-xl transition-all cursor-default group shrink-0"
              >
                <div className="text-[#F1361D] group-hover:scale-110 transition-transform">
                  {spec.icon}
                </div>
                <span className="text-xs font-black text-slate-800 whitespace-nowrap uppercase tracking-widest">{spec.name}</span>
              </div>
            ))}
          </div>
        </div>

        {/* 'Two Proven Pillars' Header */}
        <div className="mb-12 animate-in fade-in slide-in-from-top-4 duration-1000 delay-500">
          <h2 className="text-2xl md:text-[36px] text-[#1A1A1A] font-bold tracking-tight bg-white/40 backdrop-blur-sm inline-block px-4 py-2 rounded-lg">
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-10 max-w-6xl mx-auto px-4 w-full">
          ...
          <div className="bg-white/95 backdrop-blur-md border border-gray-100 rounded-3xl p-10 lg:p-12 flex flex-col items-center text-center animate-in fade-in slide-in-from-left-6 duration-1000 delay-700 group hover:shadow-2xl transition-all h-full shadow-xl">
             <div className="relative z-10 w-full flex flex-col items-center">
                <div className="w-16 h-16 bg-gradient-to-br from-[#4f86f7] to-[#2B59C3] rounded-2xl flex items-center justify-center mb-8 shadow-lg shadow-blue-500/20 transform group-hover:scale-105 transition-transform">
                  <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" />
                  </svg>
                </div>
                
                <h3 className="text-[22px] md:text-2xl font-bold text-slate-900 mb-3 tracking-tight">
                  Software Development Services
                </h3>
                
                <p className="text-slate-500 text-base md:text-lg font-medium mb-10">
          ...
          <div className="bg-white/95 backdrop-blur-md border border-gray-100 rounded-3xl p-10 lg:p-12 flex flex-col items-center text-center animate-in fade-in slide-in-from-right-6 duration-1000 delay-700 group hover:shadow-2xl transition-all h-full shadow-xl">
             <div className="relative z-10 w-full flex flex-col items-center">
                <div className="w-16 h-16 bg-gradient-to-br from-[#4f86f7] to-[#2B59C3] rounded-2xl flex items-center justify-center mb-8 shadow-lg shadow-blue-500/20 transform group-hover:scale-105 transition-transform">
                  <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
